<?php
/** A simple text block **/
class Arsene_Block_Container extends AQ_Block {
	...
	function form($instance) {	
		...
		__('Drag block items into this container box', 'arsene'),
		'</p>';	
			...
			echo '<li id="template-block-'.$number.'" class="block block-aq_column_block '.$size.'">',
					'<div class="block-settings-column cf" id="block-settings-'.$number.'">',
						'<p class="empty-column">',
							__('Drag block items into this container box', 'framework'),
						'</p>',
	...
	function after_form($instance) {
		...
		$block_saving_id = 'aq_blocks[aq_block_'.$number.']';
			...
			echo '<input type="hidden" class="id_base" name="'.$this->get_field_name('id_base').'" value="arsene_block_container" />';
